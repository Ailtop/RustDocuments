...
public class SaveRestore : SingletonComponent<SaveRestore>
{
		...
		try
		{
			...
			using (FileStream fileStream = File.OpenRead(strFilename))
			{
				using (BinaryReader binaryReader = new BinaryReader(fileStream))
				{
					...
					if (binaryReader.ReadUInt32() != 200)
					{
		...
		if (!Rust.Application.isQuitting)
		{
			...
			using (TimeWarning.New("SaveDisk", 100))
			{
				try
				{
					...
					{
						UnityEngine.Debug.LogWarning("Couldn't write save file! We got an exception: " + arg);
						if (File.Exists(strFilename + ".new"))
						{
							File.Delete(strFilename + ".new");
						}
				...
				{
					UnityEngine.Debug.LogWarning("Error when saving to disk: " + arg2);
				}
			}
			timerDisk.Stop();
	...
	[IteratorStateMachine(typeof(_003CDoAutomatedSave_003Ed__14))]
	private IEnumerator DoAutomatedSave(bool AndWait = false)
	{
		Interface.CallHook("OnServerSave");
		return new _003CDoAutomatedSave_003Ed__14(0)
		{
