	...
	public sealed class ProxyNumber : Variant
	{
		...
		private static IConvertible Parse(string value)
		{
			if (value.IndexOfAny(floatingPointCharacters) == -1)
			{
				...
				{
					long result;
					if (long.TryParse(value, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out result))
					{
						return result;
					}
				}
			...
			decimal result3;
			if (decimal.TryParse(value, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out result3))
			{
				double result4;
				if (result3 == 0m && double.TryParse(value, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out result4) && Math.Abs(result4) > double.Epsilon)
				{
					return result4;
				}
				return result3;
			...
			double result5;
			if (double.TryParse(value, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out result5))
			{
				return result5;
			}
			return 0;
