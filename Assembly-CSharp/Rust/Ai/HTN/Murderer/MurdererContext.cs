...
namespace Rust.Ai.HTN.Murderer
{
	...
	public class MurdererContext : BaseNpcContext, IDisposable
	{
		...
		public override Dictionary<Guid, Stack<Apex.Ai.HTN.IEffect>> AppliedEffects { get; set; } = new Dictionary<Guid, Stack<Apex.Ai.HTN.IEffect>>(20);


		public override Dictionary<Guid, Stack<Apex.Ai.HTN.IEffect>> AppliedExpectedEffects { get; set; } = new Dictionary<Guid, Stack<Apex.Ai.HTN.IEffect>>(20);
		...
		public override Vector3 GetDirectionToPrimaryEnemyPlayerTargetBody()
		{
			...
			if (primaryEnemyPlayerTarget.Player != null)
			{
				...
				return (vector2 - vector3).normalized;
			}
		...
		public override Vector3 GetDirectionToPrimaryEnemyPlayerTargetHead()
		{
			...
			if (primaryEnemyPlayerTarget.Player != null)
			{
				...
				return (position - vector).normalized;
			}
			return Body.transform.forward;
		}
		...
		public override Vector3 GetDirectionToMemoryOfPrimaryEnemyPlayerTarget()
		{
			...
			{
				Vector3 vector = Body.CenterPoint();
				float num = vector.y - Body.transform.position.y;
		...
		public MurdererContext(HTNPlayer body, MurdererDomain domain)
		{
			...
			Domain = domain;
			PlanState = PlanStateType.NoPlan;
			if (Memory == null || Memory.MurdererContext != this)
			{
				Memory = new MurdererMemory(this);
			}
			if (Location == null)
			{
