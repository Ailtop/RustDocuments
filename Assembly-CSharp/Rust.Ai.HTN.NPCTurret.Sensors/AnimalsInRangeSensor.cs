...
namespace Rust.Ai.HTN.NPCTurret.Sensors
{
	...
	public class AnimalsInRangeSensor : INpcSensor
	{
		...
		{
			if (npc == null || npc.transform == null || npc.IsDestroyed || npc.AiDefinition == null || QueryResults == null)
			{
				return;
			}
			NPCTurretDomain nPCTurretDomain = npc.AiDomain as NPCTurretDomain;
			if (nPCTurretDomain == null || nPCTurretDomain.NPCTurretContext == null)
			{
				return;
			}
			AttackEntity firearm = nPCTurretDomain.GetFirearm();
