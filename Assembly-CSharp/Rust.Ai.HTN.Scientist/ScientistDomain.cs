			...
			public static Vector3 GetPreferredFightingPosition(ScientistContext context, bool snapToAllowedRange = true)
			{
				...
				if (target.Player != null)
				{
					...
					else
					{
						...
						{
							a = context.Body.transform.position - target.Player.transform.position;
							magnitude = a.magnitude;
							a.Normalize();
						}
						else
						{
							a = target.Player.transform.position - context.Body.transform.position;
							magnitude = a.magnitude;
							a.Normalize();
						}
						float d = magnitude - preferredRange;
