...
namespace UnityEngine.UI
{
	...
	public class ScrollRectEx : UIBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutGroup, ILayoutController
	{
		...
		public virtual void OnDrag(PointerEventData eventData)
		{
			...
			Vector2 vector2 = CalculateOffset(vector - m_Content.anchoredPosition);
			vector += vector2;
			if (m_MovementType == MovementType.Elastic)
			{
				if (vector2.x != 0f)
				{
					vector.x -= RubberDelta(vector2.x, m_ViewBounds.size.x);
				}
				if (vector2.y != 0f)
				{
					vector.y -= RubberDelta(vector2.y, m_ViewBounds.size.y);
				}
			}
