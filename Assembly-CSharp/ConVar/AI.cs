...
namespace ConVar
{
	...
	public class AI : ConsoleSystem
	{
		...
		public static void aiLoadBalancerUpdateInterval(Arg args)
		{
		...
		public static void NpcSenseLoadBalancerUpdateInterval(Arg args)
		{
			LoadBalancedQueue loadBalancedQueue = NPCSensesLoadBalancer.NpcSensesLoadBalancer as LoadBalancedQueue;
			if (loadBalancedQueue != null)
			{
				loadBalancedQueue.defaultUpdateInterval = args.GetFloat(0, loadBalancedQueue.defaultUpdateInterval);
			}
		...
		[ServerVar]
		public static void aiDebug_LoadBalanceOverdueReportServer(Arg args)
		{
			TextTable table = new TextTable();
			table.AddColumn("type");
			table.AddColumn("count");
			table.AddColumn("overdue");
			AddLBTableEntry(ref table, "Default", Apex.LoadBalancing.LoadBalancer.defaultBalancer as LoadBalancedQueue);
			AddLBTableEntry(ref table, "Ai Manager", AiManagerLoadBalancer.aiManagerLoadBalancer as LoadBalancedQueue);
			AddLBTableEntry(ref table, "Ai Behaviour", AILoadBalancer.aiLoadBalancer as LoadBalancedQueue);
			AddLBTableEntry(ref table, "Npc Senses", NPCSensesLoadBalancer.NpcSensesLoadBalancer as LoadBalancedQueue);
			AddLBTableEntry(ref table, "Animal Senses", AnimalSensesLoadBalancer.animalSensesLoadBalancer as LoadBalancedQueue);
