	...
	public class TexasHoldEmController : CardGameController
	{
		...
		protected override void SubEndRound()
		{
			...
			foreach (CardPlayerData cardPlayerData2 in array)
			{
				...
				{
					num -= cardPlayerData2.betThisRound;
				}
			}
			...
			while (obj.Count > 1)
			{
				...
				foreach (CardPlayerData cardPlayerData3 in array)
				{
					...
					{
						if (cardPlayerData3.betThisRound < num2)
						{
							num2 = cardPlayerData3.betThisRound;
						}
						num3++;
					}
				}
				...
				{
					item.betThisRound -= num2;
				}
				int num5 = int.MaxValue;
				foreach (CardPlayerData item2 in obj)
				{
					if (item2.HasUserInCurrentRound && item2.finalScore < num5)
					{
						num5 = item2.finalScore;
					}
				}
				int num6 = 0;
				foreach (CardPlayerData item3 in obj)
				{
					if (item3.HasUserInCurrentRound && item3.finalScore == num5)
					{
						num6++;
					}
				}
				int num7 = Mathf.CeilToInt((float)(num4 + num) / (float)num6);
				num = 0;
				foreach (CardPlayerData item4 in obj)
				{
					if (item4.HasUserInCurrentRound && item4.finalScore == num5)
					{
						PayOut(item4, num7);
						_003CSubEndRound_003Eg__AddWinner_007C46_0(item4, num7, primaryWinner);
					}
				}
				for (int num8 = obj.Count - 1; num8 >= 0; num8--)
				{
					if (obj[num8].betThisRound == 0)
					{
						obj.RemoveAt(num8);
					}
				}
			...
			if (obj.Count == 1)
			{
				int betThisRound = obj[0].betThisRound;
				PayOut(obj[0], betThisRound);
				_003CSubEndRound_003Eg__AddWinner_007C46_0(obj[0], betThisRound, false);
			}
		...
		private void IncrementActivePlayer()
		{
			...
			if (num != 0)
			{
				...
				{
					SetActivePlayerIndex((activePlayerIndex + 1) % num);
				}
			}
		}
		...
		protected override void SubReceivedInputFromPlayer(CardPlayerData playerData, int input, int value, bool countAsAction)
		{
			...
			{
			case 2:
				playerData.LeaveCurrentRound(true, false);
				flag = true;
				LastActionValue = 0;
				break;
			case 4:
