...
namespace Rust.Ai.HTN.ScientistAStar
{
	...
	public class ScientistAStarDefinition : BaseNpcDefinition
	{
		...
		public IEnumerator EquipWeapon(HTNPlayer target)
		{
			...
			yield return CoroutineEx.waitForSeconds(0.25f);
			ScientistAStarDomain scientistAStarDomain = target.AiDomain as ScientistAStarDomain;
			if (!scientistAStarDomain)
			{
				yield break;
			}
			if (slot.info.category == ItemCategory.Weapon)
			{
				...
				{
					scientistAStarDomain.ScientistContext.SetFact(Facts.HeldItemType, ItemType.ProjectileWeapon);
					scientistAStarDomain.ReloadFirearm();
				}
				else if (heldEntity is BaseMelee)
				{
					scientistAStarDomain.ScientistContext.SetFact(Facts.HeldItemType, ItemType.MeleeWeapon);
				}
				else if (heldEntity is ThrownWeapon)
				{
					scientistAStarDomain.ScientistContext.SetFact(Facts.HeldItemType, ItemType.ThrowableWeapon);
				}
			}
			else if (slot.info.category == ItemCategory.Medical)
			{
				scientistAStarDomain.ScientistContext.SetFact(Facts.HeldItemType, ItemType.HealingItem);
			}
			else if (slot.info.category == ItemCategory.Tool)
			{
				scientistAStarDomain.ScientistContext.SetFact(Facts.HeldItemType, ItemType.LightSourceItem);
			}
		}
