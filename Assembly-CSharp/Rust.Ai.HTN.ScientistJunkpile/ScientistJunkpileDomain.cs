			...
			public static Vector3 GetPreferredFightingPosition(ScientistJunkpileContext context, bool snapToAllowedRange = true)
			{
				...
				if (primaryEnemyPlayerTarget.Player != null)
				{
					...
					else
					{
						...
						{
							a = context.Body.transform.position - primaryEnemyPlayerTarget.Player.transform.position;
							magnitude = a.magnitude;
							a.Normalize();
						}
						else
						{
							a = primaryEnemyPlayerTarget.Player.transform.position - context.Body.transform.position;
							magnitude = a.magnitude;
							a.Normalize();
						}
						float d = magnitude - num;
						bodyPosition = context.Body.transform.position + a * d;
					}
					Vector3 vector = bodyPosition;
					for (int i = 0; i < 10; i++)
					{
						NavMeshHit hit;
						if (NavMesh.SamplePosition(vector + Vector3.up * 0.1f, out hit, 2f * context.Domain.NavAgent.height, -1))
						{
							Vector3 position = hit.position;
						...
						{
							context.Memory.AddFailedDestination(vector);
						}
						Vector2 vector2 = UnityEngine.Random.insideUnitCircle * 5f;
						vector = bodyPosition + new Vector3(vector2.x, 0f, vector2.y);
					}
				}
				return context.Body.transform.position;
			}
