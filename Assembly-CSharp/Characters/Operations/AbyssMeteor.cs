...
namespace Characters.Operations
{
	public class AbyssMeteor : CharacterOperation
	{
		...
		private IEnumerator CFire(Character owner)
		{
			...
			{
				Fire(owner, bounds, lookingDirection);
				yield return owner.chronometer.animation.WaitForSeconds(_fireInterval.value);
			}
		}

		private void Fire(Character owner, Bounds bounds, Character.LookingDirection lookingDirection)
		{
			...
			if (_directionType == DirectionType.OwnerDirection)
			{
				for (int j = 0; j < values.Length; j++)
				{
					...
					_projectile.reusable.Spawn(list[j]).GetComponent<Projectile>().Fire(owner, _attackDamage.amount, value, flipX, flipY);
				}
				return;
			}
			for (int k = 0; k < values.Length; k++)
			{
				...
				_projectile.reusable.Spawn(list[k]).GetComponent<Projectile>().Fire(owner, _attackDamage.amount, value);
			}
		}
	}
}
